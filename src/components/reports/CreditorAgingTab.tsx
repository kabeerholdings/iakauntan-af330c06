...
const CreditorAgingTab = ({ invoices, contacts, payments, asOfDate }: Props) => {
  const asOf = new Date(asOfDate);

  const creditorData = useMemo(() => {
    const purchaseInvoices = invoices.filter(i => i.invoice_type === 'purchase' && i.status !== 'paid' && i.status !== 'cancelled');
    const contactMap = new Map(contacts.filter(c => c.type === 'supplier' || c.type === 'both').map(c => [c.id, c]));
  ...
  const totals = creditorData.reduce((s, d) => ({
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Creditor Aging Report</CardTitle></CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {creditorData.length === 0 ? (
              <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No outstanding creditor balances</TableCell></TableRow>
            ) : (
              <>
                {creditorData.map((d, i) => (
                  <TableRow key={i}>
...
export default CreditorAgingTab;
