...
import { Plus, Eye, MoreHorizontal, XCircle, FileText, Copy, Trash2 } from 'lucide-react';
...
const SalesDocumentsPage = () => {
  ...
  const fetchData = async () => {
    ...
    const [sd, con, si, tpl] = await Promise.all([
  ...
  const handleCreate = async () => {
    ...
    if (editDoc) {
      ...
      // Replace lines
      await supabase.from('sales_document_lines').delete().eq('document_id', editDoc.id);
  ...
  const handleEdit = (d: any) => {
    ...
    setForm({
      ...
      lines: (d.sales_document_lines || []).map((l: any) => ({
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    await supabase.from('sales_documents').update({ status }).eq('id', id);
  ...
  const handleConvertToInvoice = async (d: any) => {
    if (!selectedCompany) return;
    const { data: inv, error } = await supabase.from('invoices').insert({
      company_id: selectedCompany.id, invoice_number: `INV-${d.doc_number}`,
      invoice_type: 'sales', contact_id: d.contact_id, invoice_date: new Date().toISOString().split('T')[0],
      subtotal: d.subtotal, tax_amount: d.tax_amount, total_amount: d.total_amount,
      notes: d.notes, currency: d.currency,
    }).select().single();
    if (error) { toast.error(error.message); return; }
    if (inv && d.sales_document_lines?.length) {
      await supabase.from('invoice_lines').insert(
        d.sales_document_lines.map((l: any) => ({
    ...
    await supabase.from('sales_documents').update({ status: 'completed' }).eq('id', d.id);
  ...
  const handleDuplicate = (d: any) => {
    ...
    setOpen(true);
    toast.info('Duplicated — enter a new document number');
  };

  const getDocTypeLabel = (type: string) => {
    const map: Record<string, string> = { quotation: 'QUOTATION', sales_order: 'SALES ORDER', delivery_order: 'DELIVERY ORDER', credit_note: 'CREDIT NOTE', debit_note: 'DEBIT NOTE' };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl font-bold text-foreground">Sales Documents</h1>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredDocs.length === 0 ? (
                    <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No {docTypes.find(t => t.value === activeTab)?.label}s yet</TableCell></TableRow>
                  ) : filteredDocs.map(d => (
                    <TableRow key={d.id}>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {d.status === 'draft' && (
                              <DropdownMenuItem onClick={() => handleUpdateStatus(d.id, 'confirmed')}>
                                Confirm
                              </DropdownMenuItem>
                            )}
                            {d.status === 'confirmed' && (activeTab === 'quotation' || activeTab === 'sales_order') && (
                              <DropdownMenuItem onClick={() => handleConvertToInvoice(d)}>
                            ...
                            {d.status === 'cancelled' && (
                              <DropdownMenuItem onClick={() => handleUpdateStatus(d.id, 'draft')}>
                                Restore to Draft
                              </DropdownMenuItem>
                            )}
                            {d.status === 'draft' && (
      ...
      <Dialog open={open} onOpenChange={v => { setOpen(v); if (!v) setEditDoc(null); }}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="font-display">{editDoc ? 'Edit' : 'Create'} Sales Document</DialogTitle></DialogHeader>
      ...
      {previewDoc && (
        <DocumentPrintPreview
          ...
          lines={(previewDoc.sales_document_lines || []).map((l: any) => ({
